...
do {
    num = parseInt(prompt(" Ingrese un Numero para Acumular  (0 para terminar):"));
    if (num !==0) {
        numeroAcumulado += num;
    }

} while (num !== 0);
console.log("La Suma de los numeros acumulados es: "+numeroAcumulado)
...
if (divisores1.length <= 2 && primo >= 2) {
    console.log("PRIMO Divisores("+divisores1.join(", ")+")");
} else {
...
let numero1 = parseInt(prompt(" Ingrese Numero a sacar DIVISORES:"));
...
let array = ["Perro",2,"Gato",4,5<6,6,"Raton",8<3,9,"DIEZ"]
...
console.log("Array Inicial: "+array1.join(", "));
...
let arrayImpares = [];
console.log("Array Inicial: "+array2.join(", "));
for (let i = 0; i < array2.length; i++) {
    if (array2[i] % 2 !== 0) {
        arrayImpares.push(array2[i]);
    }  
}
console.log("LOS IMPARES SON: "+arrayImpares.join(", "));
...
let array3 = [10,7,3,99,55,99,2,55,99,1];
let mayor = array3[0];
console.log("Array Inicial: "+array3.join(", "));
...
let menor = array4[0];
console.log("Array Inicial: "+array4.join(", "));
for (let i = 9; i >= 0; i--) {
    if (menor >= array4[i]) {
        menor = array4[i]    
    }
...
// // Encontre otr forma der hacerlo:
// for (let x = 5; x >= 1; x--) {
//     console.log('* '.repeat(x));
// }


// 16 Fue el primer ejercicio que realice
alert("EJERCICIO 16:" + "\n" + "ARREGLO 10 NUMEROS DESORDENADOS--- DEVUELVE X CONSOLA ARREGLO ORDENADO");
let array8 = [9, 8, 7, 6, 9, 4, 3, 2, 1, 9]
let temp = null;
console.log("Array Inicial: "+array8.join(", "));
...
console.log("Array Final: "+array8.join(", "));
